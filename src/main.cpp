...
#if defined(NDEBUG)
# error "Akcecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x3facc53fcb574bf4cb2339f72c533d84209a784d7b4da12e5671eb5acd1e8750");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Akcecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Akcecoin Signed Message:\n";
...
{
    // Akcecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Akcecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Akcecoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Akcecoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Akcecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Akcecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xa0eeb6b8616073add37a08111874aa10e86e9dd55016e455a1cdd7a25a7b64db");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Vedat DRAMA in ISTANBUL 31-07-2019";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04562713243c967fd0f28f5661015661a26c70d458fc7cc67812f3a6a6298ba22cc98b444ae0df51e0dea58e801e7c6dcd9c182a75ae8f75dc98f78a6ba4bff4bb") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1564603538;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2364272;

        if (fTestNet)
        {
            block.nTime    = 1564603564;
            block.nNonce   = 808992;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x239e1f799a569ca5454844ab348cd152ff664f0e8eb3799af336ed6f5303857b"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Akcecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AkcecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AkcecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AkcecoinMiner(CWallet *pwallet)
{
    printf("AkcecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("akcecoin-miner");
    ...
    try { loop {
        ...
        printf("Running AkcecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AkcecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AkcecoinMiner, pwallet));
}
